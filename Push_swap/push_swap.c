...
/*   Updated: 2025/06/25 15:01:42 by xalves           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	free_all(t_list *a_stack, t_list *b_stack, int *aux_array)
{
	free(aux_array);
...
static void	convert_to_index_and_binary(t_list *stack, int *aux_arr, int bit_ln)
{
	while (stack)
	{
...
int	main(int argc, char const *argv[])
{
	...
	b_stack = NULL;
	if (argc - 1 <= 5)
		small_sort(&a_stack, &b_stack, argc - 1);
	else
