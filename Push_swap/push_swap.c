...
/*   Updated: 2025/07/01 12:38:40 by xalves           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	radix_sort(t_list **a, t_list **b, int bit_count)
{
	...
	while (bit < bit_count)
	{
		...
		{
			node = *a;
			// Check if the current bit (starting from LSB) is 0
			if (!(node->content & (1 << bit)))
				pb(a, b); // Push to B if bit is 0
			else
				ra(a); // Rotate A if bit is 1
		}
		while (*b)
			pa(a, b);
		bit++;
...
int	main(int argc, char const *argv[])
{
	...
	else
	{
		...
		convert_to_index_and_binary(a_stack, aux_array, bigg_bit_len);
